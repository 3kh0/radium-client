...
const char* HitEffects::getModuleName() {
	...
	if (sound.getSelectedValue() == 2) name = string("HitEffects ") + string(GRAY) + string("CowBell");
	return name.c_str();
}
	...
	if (g_Data.canUseMoveKeys() && !targetList.empty()) {
		...
		if (targetList[0]->damageTime > 9) {
			...
			/*case 2: // LOL
				...
				break;*/
			case 2: // Pop
				level->playSound("note.cow_bell", *player->getPos(), volume / 10, 1);
				break;
			}
		}
	}
